...
.as{background:#E8F5E9;border-left:4px solid #43A047;color:#2E7D32}
...
.rtc-status{display:inline-block;padding:3px 9px;border-radius:20px;font-size:11px;font-weight:700}
.rtc-ok{background:#E8F5E9;color:#2E7D32}
...
<div class="ibox"><div class="l">DS3231 Temp</div><div class="v" id="stmp">&hellip;</div></div>
</div>

<div class="card fcrd" style="margin-bottom:14px">
<div class="alert as" id="rtcStatusAlert" style="display:none"></div>
...
function loadSys(){
  fetch('/api/system').then(r=>r.json()).then(d=>{
    ...
    const rtcDiv = document.getElementById('srtc');
    const tmpDiv = document.getElementById('stmp');
    const rtcAlert = document.getElementById('rtcStatusAlert');
...
void syncFromDS3231() {
    ...
    if (ds3231Epoch < 1577836800UL) return;    
    ...
    if (rtcInitialized && internalEpoch > 0) {
        ...
        if (elapsedMs > 60000UL && elapsedMs < MAX_DRIFT_CALIBRATION_INTERVAL) {
            ...
            double actualSecs = (double)((long)ds3231Epoch - (long)internalEpoch);
            double measuredRate = actualSecs / nominalSecs;            
            if (measuredRate > 0.5 && measuredRate < 2.0) {
                driftCompensation = driftCompensation * 0.9f + measuredRate * 0.1f;                
...
void selfRecoveryRTC() {
    ...
    if (ds3231Present) {
        syncFromDS3231();
    } else if (wifiConnected && ESP.getFreeHeap() > MIN_HEAP_FOR_NTP) {
        tryNTPSync();
    }
}
void selfRecoveryMemoryCleanup() {
...
void updateStatusLED() {
    ...
    } else if (!rtcInitialized || (lastNTPSync == 0 && lastNTPSyncEpoch == 0 && lastDS3231SyncEpoch == 0)) {
...
void proactiveMaintenance() {
    ...
    if (ds3231Present && elapsedSince(lastDS3231Sync) > 60000UL) {
        ds3231LastReadTime = rtc.now();
    }
}
...
void initDefaults() {
    ...
    strcpy(sysConfig.ap_ssid, "ESP8266_6CH_Timer_Switch");
...
uint32_t extractJsonUInt32(const String& json, const char* key) {
    ...
    int digits = 0;
    while (idx < (int)json.length() && json[idx] >= '0' && json[idx] <= '9' && digits < 10) {
        val = val * 10 + (uint32_t)(json[idx] - '0');
        idx++;
        digits++;
    }
    return val;
...
void handleSaveRelay() {
    ...
    for (JsonObject sc : schedules) {
        if (s >= 8) break;
        relayConfigs[relay].schedule.startHour[s]   = sc["startHour"]   | 0;
        relayConfigs[relay].schedule.startMinute[s] = sc["startMinute"] | 0;
        relayConfigs[relay].schedule.startSecond[s] = sc["startSecond"] | 0;
        relayConfigs[relay].schedule.stopHour[s]    = sc["stopHour"]    | 0;
        relayConfigs[relay].schedule.stopMinute[s]  = sc["stopMinute"]  | 0;
        relayConfigs[relay].schedule.stopSecond[s]  = sc["stopSecond"]  | 0;
        relayConfigs[relay].schedule.enabled[s]     = sc["enabled"]     | false;
        relayConfigs[relay].schedule.days[s]        = sc["days"]        | (uint8_t)0x7F;
        relayConfigs[relay].schedule.monthDays[s]   = sc["monthDays"]   | (uint32_t)0;
        s++;
    }
    markConfigDirty();
    server.send(200, "application/json", "{\"success\":true}");
...
void handleGetTime() {
    ...
    bool ntpSynced = (lastNTPSync > 0 || lastNTPSyncEpoch > 0 || lastDS3231SyncEpoch > 0);
...
void handleGetSystem() {
    ...
    if (shouldSimplifyResponse()) {
        ...
        snprintf(buf, sizeof(buf), 
            "{\"freeHeap\":%d,\"uptime\":%lu,\"wifiConnected\":%s,\"ds3231Present\":%s}",
    ...
    unsigned long syncAge = lastNTPSync > 0 ? elapsedSince(lastNTPSync) / 1000UL : (lastNTPSyncEpoch > 0 ? (millis() / 1000UL) : 0);
